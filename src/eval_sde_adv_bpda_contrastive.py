# the original license of this file is as follows:
# ---------------------------------------------------------------
...
def default_constructive_plug_in(args, model: nn.Module, device=None):
    return ContrastiveGuidedDiffusion(
        model,
        get_image_classifier(args.contrastiveive_classifier).eval().to(device),
...
class SDE_Adv_Model(nn.Module):
    def __init__(self, args, config):
        ...
        if args.diffusion_type == 'ddpm':
            ...
            _model = default_constructive_plug_in(args, self.runner.model, device=config.device)
...
def parse_args_and_config():
    ...
    # fixed error
    parser.add_argument('--fix_rand', type=str2bool, default=False, help='fix random seed')

    # constructive args
    parser.add_argument('--contrastiveive_classifier', type=str, default="cifar10-wrn-70-16-dropout", help='which classifier to use for contrastiveive guided diffusion')
    parser.add_argument('--constructive_n_classes', type=int, default=10, help='number of classes for contrastiveive guided diffusion')
    parser.add_argument('--constructive_optim_lr', type=float, default=0.01, help='learning rate for contrastiveive guided diffusion')
    parser.add_argument('--constructive_drift_min', type=int, default=40, help="minimun step for computing drift start")
    parser.add_argument('--constructive_drift_max', type=int, default=100000, help="maximum step for computing drift end")
    parser.add_argument('--contrastive_tau_plus', type=float, default=0.1, help='tau_plus for contrastiveive guided diffusion')
    parser.add_argument('--contrastive_beta', type=float, default=1.0, help='beta for contrastiveive guided diffusion')
    parser.add_argument('--contrastive_temperature', type=float, default=10, help='temperature for contrastiveive guided diffusion')
    parser.add_argument('--contrastive_delta_grad_mul', type=float, default=5000, help='delta_grad_mul for contrastiveive guided diffusion')
    parser.add_argument('--contrastive_estimator', type=str, default='hard', help='estimator for contrastiveive guided diffusion')
