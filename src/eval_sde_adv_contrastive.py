...
from contrastive_guided_diffusion import ContrastiveGuidedDiffusion

from eval_sde_adv import eval_eotpgd, eval_apgd


def default_constructive_plug_in(args, model: nn.Module, device=None):
    img_shape = None
    ...
    return ContrastiveGuidedDiffusion(
        model,
        get_image_classifier(args.contrastiveive_classifier).eval(),
        n_classes=args.constructive_n_classes,
        batch_size=args.num_sub,
        img_shape=img_shape,
        optim_lr=args.constructive_optim_lr,
        enable_drift=True,
        drift_counter_min=args.constructive_drift_min,
        drift_counter_max=args.constructive_drift_max,  # just a large number
        tau_plus=args.contrastive_tau_plus,
        beta=args.contrastive_beta,
        temperature=args.contrastive_temperature,
        delta_grad_mul=args.contrastive_delta_grad_mul,
        estimator=args.contrastive_estimator,
    ).to(device)

class SDE_Adv_Model(nn.Module):
    def __init__(self, args, config, debug_label=None):
        super().__init__()
        self.args = args
        ...
        self.classifier = get_image_classifier(
            args.classifier_name).to(config.device)
        ...
        if args.diffusion_type == 'ddpm':
            ...
            _model = default_constructive_plug_in(
                args, self.runner.model, device=config.device)
            self.runner.model = _model
        elif args.diffusion_type == 'sde':
            self.runner = RevGuidedDiffusion(
                args, config, device=config.device)
            _model = default_constructive_plug_in(
                args, self.runner.model, device=config.device)
            self.runner.model = _model
            self.runner.rev_vpsde.model = _model
        elif args.diffusion_type == 'ode':
            self.runner = OdeGuidedDiffusion(
                args, config, device=config.device)
            _model = default_constructive_plug_in(
                args, self.runner.model, device=config.device)
            self.runner.model = _model
            self.runner.vpode.model = _model
        ...
        print("\033[34m inserted constructive guide\033[0m")
    ...
    def forward(self, x):
        ...
        out = self.classifier(((input_x + _w) + 1) * 0.5)

        self.counter += 1

        if isinstance(self.runner.model, ContrastiveGuidedDiffusion):
            self.runner.model.reset_drift_counter()
        # if self.debug_label is not None:
        #     acc = (out.max(1)[1] == self.debug_label.to(out.device)).float().mean()
        #     print(f"\033[33m[debug acc] {counter} {acc}\033[0m")
...
def eval_autoattack(args, config, model, x_val, y_val, adv_batch_size, log_dir):
    ...
    if attack_version == 'custom':
        ...
        # In the first run, we set it to 555
        # since the attack_version in run_scripts are standard,
        # this will not effect the result.
        # adversary_resnet.square.n_queries = 555 # 5000 -> 555
        adversary_resnet.square.n_queries = 5000  # 5000 -> 555
    ...
    x_adv_resnet = adversary_resnet.run_standard_evaluation(
        x_val, y_val, bs=adv_batch_size, state_path=f"{log_dir}/adv_resnet_eval_state.json")
    print(f'x_adv_resnet shape: {x_adv_resnet.shape}')
    torch.save([x_adv_resnet, y_val],
               f'{log_dir}/x_adv_resnet_sd{args.seed}.pt')
    ...
    if attack_version == 'rand':
        ...
        print(
            f'[adv_sde] rand version with eot_iter: {adversary_sde.apgd.eot_iter}')
    print(f'{args.lp_norm}, epsilon: {args.adv_eps}')

    x_adv_sde = adversary_sde.run_standard_evaluation(
        x_val, y_val, bs=adv_batch_size, state_path=f"{log_dir}/adv_sde_eval_state.json")
...
def eval_stadv(args, config, model, x_val, y_val, adv_batch_size, log_dir):
    ...
    print('initial accuracy: {:.2%}, time elapsed: {:.2f}s'.format(
    ...
    print('robust accuracy: {:.2%}, time elapsed: {:.2f}s'.format(
...
def robustness_eval(args, config):
    ...
    ngpus = torch.cuda.device_count()
    if ngpus > 1 and (args.contrastive_data_drift != -1):
        raise ValueError(
            'contrastive_data_drift is not supported for multi-gpu')

    adv_batch_size = args.adv_batch_size * ngpus
    print(f'ngpus: {ngpus}, adv_batch_size: {adv_batch_size}')

    # load data
    if args.contrastive_data_drift < 0:
        x_val, y_val = load_data(args, adv_batch_size)
    else:
        x_val, y_val = load_data(
            args, (args.contrastive_data_drift + 1) * adv_batch_size)

        _drift = args.contrastive_data_drift
        _ll = _drift * args.adv_batch_size
        _rr = _ll + args.adv_batch_size

        print(f"use data drift mode; pick x_val[{_ll}:{_rr}]")
    ...
    if args.attack_version in ['standard', 'rand', 'custom']:
        eval_autoattack(args, config, model, x_val,
                        y_val, adv_batch_size, log_dir)
    elif args.attack_version == 'stadv':
        eval_stadv(args, config, model, x_val, y_val, adv_batch_size, log_dir)
    elif args.attack_version == 'eotpgd':
        eval_eotpgd(args, config, model, x_val, y_val, adv_batch_size, log_dir)
    elif args.attack_version == 'apgd':
        eval_apgd(args, config, model, x_val, y_val, adv_batch_size, log_dir)
    else:
        raise NotImplementedError(
            f'unknown attack_version: {args.attack_version}')
...
def parse_args_and_config():
    ...
    parser.add_argument('--sample_step', type=int,
                        default=1, help='Total sampling steps')
    parser.add_argument('--t', type=int, default=400,
                        help='Sampling noise scale')
    parser.add_argument('--t_delta', type=int, default=15,
                        help='Perturbation range of sampling noise scale')
    parser.add_argument('--rand_t', type=str2bool, default=False,
                        help='Decide if randomize sampling noise scale')
    parser.add_argument('--diffusion_type', type=str,
                        default='ddpm', help='[ddpm, sde]')
    parser.add_argument('--score_type', type=str,
                        default='guided_diffusion', help='[guided_diffusion, score_sde]')
    parser.add_argument('--eot_iter', type=int, default=20,
                        help='only for rand version of autoattack')
    parser.add_argument('--use_bm', action='store_true',
                        help='whether to use brownian motion')

    # LDSDE
    parser.add_argument('--sigma2', type=float,
                        default=1e-3, help='LDSDE sigma2')
    parser.add_argument('--lambda_ld', type=float,
                        default=1e-2, help='lambda_ld')
    parser.add_argument('--eta', type=float, default=5., help='LDSDE eta')
    parser.add_argument('--step_size', type=float, default=1e-3,
                        help='step size for ODE Euler method')

    # adv
    parser.add_argument('--domain', type=str, default='celebahq',
                        help='which domain: celebahq, cat, car, imagenet')
    parser.add_argument('--classifier_name', type=str,
                        default='Eyeglasses', help='which classifier to use')
    parser.add_argument('--partition', type=str, default='val')
    parser.add_argument('--adv_batch_size', type=int, default=64)
    parser.add_argument('--attack_type', type=str, default='square')
    parser.add_argument('--lp_norm', type=str,
                        default='Linf', choices=['Linf', 'L2'])
    parser.add_argument('--attack_version', type=str, default='custom')

    parser.add_argument('--num_sub', type=int,
                        default=1000, help='imagenet subset')
    parser.add_argument('--adv_eps', type=float, default=0.07)

    # fixed error
    parser.add_argument('--fix_rand', type=str2bool,
                        default=False, help='fix random seed')

    # constructive args
    parser.add_argument('--contrastiveive_classifier', type=str, default="cifar10-wrn-70-16-dropout",
                        help='which classifier to use for contrastiveive guided diffusion')
    parser.add_argument('--constructive_n_classes', type=int, default=10,
                        help='number of classes for contrastiveive guided diffusion')
    parser.add_argument('--constructive_optim_lr', type=float, default=0.01,
                        help='learning rate for contrastiveive guided diffusion')
    parser.add_argument('--constructive_drift_min', type=int,
                        default=40, help="minimun step for computing drift start")
    parser.add_argument('--constructive_drift_max', type=int,
                        default=100000, help="maximum step for computing drift end")

    parser.add_argument('--contrastive_tau_plus', type=float, default=0.1,
                        help='tau_plus for contrastiveive guided diffusion')
    parser.add_argument('--contrastive_beta', type=float, default=1.0,
                        help='beta for contrastiveive guided diffusion')
    parser.add_argument('--contrastive_temperature', type=float, default=1,
                        help='temperature for contrastiveive guided diffusion')
    parser.add_argument('--contrastive_delta_grad_mul', type=float, default=500,
                        help='delta_grad_mul for contrastiveive guided diffusion')
    parser.add_argument('--contrastive_estimator', type=str, default='hard',
                        help='estimator for contrastiveive guided diffusion')
    parser.add_argument('--contrastive_data_drift', type=int,
                        default=-1, help='__developer_only__')
