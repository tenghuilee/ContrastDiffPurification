...
from runners.diffpure_sde_gdmp import RevGuidedDiffusion_GDMP
import torchattacks
...
class SDE_Adv_Model(nn.Module):
    def __init__(self, args, config):
        ...
        self.enable_label_guided_diffusion = False
        ...
        if args.diffusion_type == 'ddpm':
            ...
            self.runner = OdeGuidedDiffusion(
                args, config, device=config.device)
        elif args.diffusion_type == 'sde-gdmp':
            self.runner = RevGuidedDiffusion_GDMP(args, config, device=config.device)
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            if self.enable_label_guided_diffusion:
                guided_logists = self.classifier((x + 1) * 0.5)
                guided_labels = torch.argmax(guided_logists, dim=1)

                x_re = self.runner.image_editing_sample(
                    (x - 0.5) * 2, bs_id=counter, tag=self.tag, 
                    model_kwargs={
                        "guided_label": guided_labels,
                    }
                )
            else:
                x_re = self.runner.image_editing_sample(
...
def eval_eotpgd(args, config, model, x_val, y_val, adv_batch_size, log_dir):
    ngpus = torch.cuda.device_count()
    model_ = model
    if ngpus > 1:
        model_ = model.module
    ...
    # apply the attack to resnet
    print(f'apply the eotpgd attack to resnet...')
    resnet = get_image_classifier(args.classifier_name).to(config.device)
    ...
    adversary_resnet = torchattacks.EOTPGD(
        resnet, eps=args.adv_eps, alpha=2.0/255, steps=10, eot_iter=args.eot_iter)
    ...
    #
    print(f'apply the eotpgd attack to sde_adv...')
    ...
    adversary_sde = torchattacks.EOTPGD(
        model, eps=args.adv_eps, alpha=2.0/255, steps=10, eot_iter=args.eot_iter)
...
def eval_apgd(args, config, model, x_val, y_val, adv_batch_size, log_dir):
    ngpus = torch.cuda.device_count()
    model_ = model
    if ngpus > 1:
        model_ = model.module
    ...
    # apply the attack to resnet
    print(f'apply the apgd-ce attack to resnet...')
    resnet = get_image_classifier(args.classifier_name).to(config.device)
    ...
    adversary_resnet = torchattacks.APGD(
        resnet, eps=args.adv_eps, steps=10, eot_iter=args.eot_iter, verbose=True)
    ...
    #
    print(f'apply the apgd attack to sde_adv...')
    ...
    adversary_sde = torchattacks.APGD(
        model, eps=args.adv_eps, steps=10, eot_iter=args.eot_iter, verbose=True)
...
def parse_args_and_config():
    ...
    # instruct by reviewer
    parser.add_argument("--fix_rand", type=str2bool,
                        default=False, help="fix initial randomness")

    # for adversial guided label
    parser.add_argument("--class_cond_scale", type=float,
                        default=1e-4, help="class_cond_scale")
