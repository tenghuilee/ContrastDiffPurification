...
class RevVPSDE_GDMP(torch.nn.Module):
    def __init__(
        self,
        model,
        ...
        img_shape=(3, 256, 256),
        model_kwargs=None,
        # perturbation, pixel scale; the original implementation is = ptb(=8) * 2 / 255
        adv_eps=8/255,
        guide_scale=1000,
    ):
        ...
        super().__init__()
        self.model = model
        self.image_adv = image_adv.detach().clone()
        # guide mode = MSE
        ...
        self.sqrt_1m_alphas_cumprod_neg_recip_cont = lambda t: - \
            1. / torch.sqrt(1. - self.alphas_cumprod_cont(t))

        self.noise_type = "diagonal"
        self.sde_type = "ito"
    
    def to(self, *args, **kwargs):
        ans = super().to(*args, **kwargs)
        self.discrete_betas = self.discrete_betas.to(*args, **kwargs)
        ...
        self.sqrt_1m_alphas_cumprod = self.sqrt_1m_alphas_cumprod.to(*args, **kwargs)
        return ans

    def _scale_timesteps(self, t):
        assert torch.all(t <= 1) and torch.all(
            t >= 0), f't has to be in [0, 1], but get {t} with shape {t.shape}'
    ...
    def rvpsde_fn(self, t, x, return_type='drift'):
        ...
        if return_type == 'drift':
            ...
            assert x.ndim == 2 and np.prod(
                self.img_shape) == x.shape[1], x.shape
            ...
            if self.score_type == 'guided_diffusion':
                ...
                score = _extract_into_tensor(
                    self.sqrt_1m_alphas_cumprod_neg_recip_cont, t, x.shape) * model_output

            elif self.score_type == 'score_sde':
                # model output is epsilon
                sde = sde_lib.VPSDE(beta_min=self.beta_0,
                                    beta_max=self.beta_1, N=self.N)
            ...
            # implement GDMP with guide mode = MSE
            with torch.enable_grad():
                x_in = x.detach().requires_grad_(True)
                x_adv_t = self.diffuse_t_steps(self.image_adv.detach().view(x.shape[0], -1), disc_steps)

                # MSE
                selected = -1 * torchF.mse_loss(x_in, x_adv_t)
                scale = self.compute_grad_scale(disc_steps)


            delta = torch.autograd.grad(selected, x_in)[0] * scale
            # print(f"[{disc_steps[0].item()}] {selected} {torch.norm(delta).item()}")
    ...
    def compute_scale(self, t, m):
        alpha_bar = self.alphas_cumprod[t].reshape(-1, 1)
        return torch.sqrt(1-alpha_bar) / (m*torch.sqrt(alpha_bar))

    def compute_grad_scale(self, t):
        # . adv_eps = 8/255
        # original implementation
        # ptb(=8) * 2 / 255 / 3 / guide_scale
        m = self.adv_eps * 2 / 3 / self.guide_scale
        return self.compute_scale(t, m)

    def diffuse_t_steps(self, x0, t):
        # x is a torch tensor of shape (B,C,H,W)
        # t is a interger range from 0 to T-1
        alpha_bar = self.alphas_cumprod[t].reshape(-1, 1) # type: torch.Tensor
        # print(alpha_bar.shape, x0.shape)
        xt = torch.sqrt(alpha_bar) * x0 + torch.sqrt(1-alpha_bar) * \
            torch.randn_like(x0)
        return xt
...
class RevGuidedDiffusion_GDMP(torch.nn.Module):
    def __init__(self, args, config, device=None):
        ...
        if config.data.dataset.lower() == 'imagenet':
            ...
            img_shape = (3, 32, 32)
            model_dir = 'pretrained/cifar100'
            print(f'model_config: {config}')
            model = mutils.create_model(config)  # type: torch.nn.Module
            state_dict = torch.load(
                f'{model_dir}/cifar100_ncsnpp_e7974_step390775.ckpt', map_location='cpu')
        ...
        elif config.data.dataset.lower() == 'gtsrb':
            img_shape = (3, 32, 32)
            model_dir = 'pretrained/gtsrb'
            print(f'model_config: {config}')
            model = mutils.create_model(config)
            state_dict = torch.load(
                f'{model_dir}/gtsrb_score_sde.pt', map_location='cpu')
            model.load_state_dict(state_dict)
    ...
    def image_editing_sample(self, img, bs_id=0, tag=None):
        ...
        self_betas = self_rev_vpsde.discrete_betas.float().to(self.device)
        ...
        out_dir = os.path.join(
            self.args.log_dir, 'bs' + str(bs_id) + '_' + tag)
        ...
        if bs_id < 2:
            ...
            tvu.save_image(
                (x0 + 1) * 0.5, os.path.join(out_dir, f'original_input.png'))
        ...
        for it in range(self.args.sample_step):
            ...
            if self.args.rand_t:
                total_noise_levels = self.args.t + \
                    np.random.randint(-self.args.t_delta, self.args.t_delta)
                print(f'total_noise_levels: {total_noise_levels}')
            a = (1 - self_betas).cumprod(dim=0).to(self.device)
            x = x0 * a[total_noise_levels - 1].sqrt() + e * \
                (1.0 - a[total_noise_levels - 1]).sqrt()
            ...
            if self.args.use_bm:
                bm = torchsde.BrownianInterval(t0=t0, t1=t1, size=(
                    batch_size, state_size), device=self.device)
                xs_ = torchsde.sdeint_adjoint(
                    self_rev_vpsde, x_, ts, method='euler', bm=bm)
            else:
                xs_ = torchsde.sdeint_adjoint(
                    self_rev_vpsde, x_, ts, method='euler')
